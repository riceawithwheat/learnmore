...
export default defineComponent({
  ...
  setup () {
    ...
    const currentPost = computed<PostProps>(() => store.getters.getCurrentPost(route.query.id))
    // if (!currentPost.value) {
    //   currentPost.value[0] = null
    // }
    // const data = {
    //   titleVal: currentPost.value[0].title ? currentPost.value[0].title : 'null',
    //   contentVal: currentPost.value[0].content ? currentPost.value[0].content : null,
    //   profileVal: currentPost.value[0].profile ? currentPost.value[0].profile : null,
    //   url: currentPost.value[0].url ? currentPost.value[0].url : ' '
    // }
    ...
    const onFormSubmit = (result: boolean) => {
      ...
      if (result && editorStatus.isValid) {
        ...
        const sendData = isEditMode ? updateData : newPost
